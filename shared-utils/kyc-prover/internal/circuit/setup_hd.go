...
// SetupHD performs the Groth16 trusted setup ceremony for KYC-HD v1
// WARNING: This is for TESTING ONLY. Production must use a proper MPC ceremony.
func SetupHD() (*SetupResult, error) {
	...
	// Print circuit stats
	fmt.Printf("  Circuit stats: %d constraints, %d public inputs, %d secret inputs\n",
	...
	// Serialize VK in custom format for C++ compatibility
	...
	return &SetupResult{
		...
		VKHex:           "0x" + fmt.Sprintf("%x", vkCustomBytes),
		VKGnarkHex:      "0x" + fmt.Sprintf("%x", vkBuf.Bytes()),
	}, nil
}
