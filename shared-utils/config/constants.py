# SPDX-License-Identifier: Apache-2.0
# --------------------------------------------------------------------------- #
#                               hyper-parameters                              #
# --------------------------------------------------------------------------- #
import os
try:
    import torch
    from torch.distributions.normal import Normal        # already imported below
    from torch.distributions import StudentT
except ImportError:
    pass


POW_WINDOW_SIZE = 256
TOPK_MIN        = 5
TOPK_MAX        = 50
TOPP_MIN        = 0.1
TOPP_MAX        = 1
TEMP_MIN        = 0.25
TEMP_MAX        = 2
ENTROPY_FILT    = .925
REUSE_SCORE_Q32_BITS = 32
REUSE_SCORE_Q32_ONE = 1 << REUSE_SCORE_Q32_BITS
REUSE_SCORE_CAP_Q32 = 247891519322  # ceil(57.716742 * 2^32)
# Proofs with version < REUSE_GATE_VERSION are legacy/grandfathered (reuse gate is a
# no-op); version >= REUSE_GATE_VERSION are enforced. Mirrors C++ quick_verifier.h.
REUSE_GATE_VERSION = 2
REP_PENALTY     = 0.1
CACHE_DIR       = '/data/pow_proofs/cache/'
SMELL_TEST      = True
DEBUG           = False
ATOL            = 0.0001

DISCRIMINANT_SIZE = 1024

MODEL_VERIFICATION_BLOCK_COUNT = 100
SUCCESSFUL_COMMITS_COUNT = 50
MODEL_CHALLENGE_DEPOSIT = 5

# ZMQ Configuration
ZMQ_VERIFY_PULL_PORT = int(os.getenv("ZMQ_VERIFY_PULL_PORT", "6001"))
ZMQ_VERIFY_RECV_TIMEOUT_MS = int(os.getenv("ZMQ_VERIFY_RECV_TIMEOUT_MS", "6000000"))
ZMQ_VERIFY_RETRY_ATTEMPTS   = int(os.getenv("ZMQ_VERIFY_RETRY_ATTEMPTS",   "10"))
ZMQ_VERIFY_RETRY_BACKOFF    = float(os.getenv("ZMQ_VERIFY_RETRY_BACKOFF",    "1.0"))

# ZMQ Configuration
ZMQ_VERIFY_PUSH_HOST = os.getenv("ZMQ_VERIFY_PUSH_HOST", "0.0.0.0")
ZMQ_VERIFY_PUSH_PORT = int(os.getenv("ZMQ_VERIFY_PUSH_PORT", "7001"))

# VDF Configuration
VDF_DISCRIMINANT_SIZE = int(os.getenv("VDF_DISCRIMINANT_SIZE", "1024"))
VDF_CHECKPOINT_SIZE = int(os.getenv("VDF_CHECKPOINT_SIZE", "32768"))
VDF_UPDATE_INTERVAL = float(os.getenv("VDF_UPDATE_INTERVAL", "0.1"))

# Defaults (mid-range or typical values)
DEFAULT_TOP_K = 50
DEFAULT_TOP_P = 1.0
DEFAULT_TEMP = 1.0

try:
    # Bytes per element for common dtypes
    _DTYPE_BYTES = {
        torch.float16: 2,
        torch.bfloat16: 2,
        torch.float32: 4,
        torch.int8: 1,
    }

    # Map safetensors dtype codes → torch.dtype
    SF_DTYPES = {
        'F32': torch.float32,
        'F16': torch.float16,
        'BF16': torch.bfloat16,
        'F64': torch.float64,
        'I8': torch.int8,
        'I16': torch.int16,  
        'I32': torch.int32,
        'I64': torch.int64,
        'U8': torch.uint8,
        'BOOL': torch.bool,
    }

    ENUM_PROOF_OUTCOMES = {
        'RED': 1,
        'AMBER': 2,
        'GREEN': 0,
        'VALIDATING': 3,
        'ERROR': 4,
    }

    _NORMAL = Normal(0.0, 1.0)
    _TWO_PI = 2.0 * torch.pi
    pi = torch.pi
except:
    pass  # If torch is not available, skip these definitions
