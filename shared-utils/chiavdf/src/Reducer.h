...
// mpz_mul_si takes a 'long' multiplier, which is only 32-bit on Windows LLP64.
// This helper safely multiplies an mpz by a 64-bit signed integer on all platforms.
inline void mpz_mul_si64(mpz_t rop, const mpz_t op, int_fast64_t si) {
    // Fast path: value fits in long — use native mpz_mul_si
    if (si >= LONG_MIN && si <= LONG_MAX) {
        mpz_mul_si(rop, op, static_cast<long>(si));
        return;
    }
    // Slow path: set a temp mpz from the 64-bit value, then multiply.
    // Use ~x + 1 instead of -x to avoid UB on INT64_MIN.
    mpz_t tmp;
    mpz_init(tmp);
    if (si >= 0) {
        uint_fast64_t v = static_cast<uint_fast64_t>(si);
        mpz_import(tmp, 1, -1, sizeof(v), 0, 0, &v);
    } else {
        uint_fast64_t pos = ~static_cast<uint_fast64_t>(si) + 1u;
        mpz_import(tmp, 1, -1, sizeof(pos), 0, 0, &pos);
        mpz_neg(tmp, tmp);
    }
    mpz_mul(rop, op, tmp);
    mpz_clear(tmp);
  ...
  void run() {
    while (!isReduced()) {
      ...
      }
      {
        ...
        mpz_mul_si64(ctx.fba, ctx.a, u * v);
        mpz_mul_2exp(ctx.fba, ctx.fba, 1);
        mpz_mul_si64(ctx.fbb, ctx.b, u * x + v * w);
        mpz_mul_si64(ctx.fbc, ctx.c, w * x);
        mpz_mul_2exp(ctx.fbc, ctx.fbc, 1);
