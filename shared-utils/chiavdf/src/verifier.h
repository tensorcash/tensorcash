...
{
    // Reject discriminants larger than the compiled form size to prevent
    // out-of-bounds reads in bqfc_deserialize_only (it computes byte
    // counts from D.num_bits(), not from BQFC_FORM_SIZE).
    if (D.num_bits() > BQFC_MAX_D_BITS) return false;
...
integer GetBFromProof(integer D, const uint8_t* x_s, const uint8_t* proof_blob, int32_t proof_blob_len, uint64_t iterations, int32_t depth) {
    if (D.num_bits() > BQFC_MAX_D_BITS) throw std::runtime_error("Invalid proof.");
    int form_size = BQFC_FORM_SIZE;
    int segment_len = 8 + B_bytes + form_size;
